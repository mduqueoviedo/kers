...
| ID and title | Status | Objective and user value | Concepts and scope | Acceptance and testing | Dependencies, priority, and notes |
| --- | --- | --- | --- | --- | --- |
| DEP-001 — Deploy the current KERS demo to Railway | In review | Make the current locally tested application publicly accessible before continuing lower-priority features. | Minimal `railway.json`; GitHub-connected Railway application and PostgreSQL services; production environment variables; native Railpack build and start; explicit non-interactive pre-deploy migration and repeatable seeding; public URL and automatic `main` deployments. Repository preparation and manual deployment verification remain one focused PR. | The public Railway URL serves KERS with `APP_ENV=production` and debug disabled; the application uses private Railway PostgreSQL with all migrations applied; enough safe data exists to exercise current flows; assets load; GitHub Actions passes; a later `main` merge deploys automatically; no paid or unnecessary infrastructure is introduced. | INC-003; High; current pull request. Temporary for approximately one week and expected to use zero-cost Railway trial credit. Repository configuration is present; manual service creation and verification remain. |
