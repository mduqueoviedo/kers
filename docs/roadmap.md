...
| KAI-003B — Simplify the public application layout | In review | Show only implemented KERS functionality and remove confusing starter UI. | Root redirect, shared Blade layout, route and view cleanup. | The root redirects to the catalogue; its layout has simple KERS navigation; generated welcome, dashboard, settings, sidebar, and related dead components are removed; tests and frontend build pass. | KAI-003A; Medium; this pull request. |
| KAI-003C — Create kaijus | Planned | Let anyone register a known creature. | Single-file Livewire form, validation, enum rule, Eloquent creation, redirect and feedback. | Valid records persist and appear in the catalogue; required, enum, threat-level, and optional-description cases are tested. | KAI-003B; High; simple UI only. |
